...
\input{../../../../Other/latex/preamble.tex}
...
The  statistical goal of such problems is to estimate a discriminant boundary that optimally partitions the space. To do so, one must make a series of decisions to minimize the expected error.  First, one can choose which assumptions to make about the \emph{data generating mechanism}.  These properties imply geometric constraints on the set of optimal discriminant boundaries.  For example, if one assumes that the data are sampled from a Gaussian, and the two classes differ only in their means, then one can prove that the optimal discriminant boundary is linear, but we do not know the (potentially multidimensional) slope and intercept.  Second, given such assumptions, one can then choose the \emph{boundary search space} over which one will search for the optimal boundary.  For example, if we assume the optimal boundary is linear, we can search only for linear boundaries, and exclude multi-linear and quadratic boundaries.  The minimum distance between the boundaries we search over and the optimal boundary is called the \textbf{model bias}, which we want to minimize by choosing a search space as close as possible to the optimal boundary space.  Third, given our constrained search space, we can choose an \emph{algorithm} to find an optimal boundary, such as Fisher’s Linear Discriminant, Linear Support Vector Machines, or Logistic Regression.  Each such algorithm has different convergence properties as sample size increases.  Importantly, for different data generating mechanisms, different algorithms may converge on different solutions.  The additional distance between the expected estimated boundary and the optimal boundary is called the \textbf{estimator bias}, which we also want to minimize by choosing an algorithm that will select a boundary as close as possible to the truth.  Different algorithms will also differ in the \textbf{variance} of their estimated boundaries, which is a function both of the size of the search space, and the idiosyncrasies of the algorithm.  The two bias terms and the variance together determine expected square error, so, one also desires to minimize the variance of the estimated boundary.  

In addition to the above statistical considerations, there are a number of computational considerations to consider which also depend on a series of choices. In this big data age, the practical importance of these considerations is quickly gaining on the importance of the various statistical considerations.  First, the choice of \emph{implementations} for a particular algorithm can dramatically change the computational properties of an algorithm.  In particular, one desires an implementation that is \textbf{numerically stable}, regardless of the number of samples and dimensions.  Second, one desires an implementation that minimizes computational complexity, both in terms of worst case \textbf{space} and \textbf{time}.  Second, the \emph{platform} upon which the implementation runs will determine a number of other factors, including both software dependencies and hardware choice. In particular the \textbf{scalability} of the implementation, that is, how large of a problem can be solved on a particular hardware configuration.

In addition to the above theoretical considerations, there are also a number of pragmatic ones. Using theory we can prove properties of certain choices with respect to various assumptions, and use those theoretical properties to compare different methodologies.  But the theory is limited to certain relatively simple sets of assumptions, assumptions for which many real data sets and algorithms do not satisfy. Even when the assumptions are met, theory often merely provides bounds, without concrete numbers in any particular situations. Numerical experiments can therefore buttress the theoretical considerations.  In particular, \textbf{simulated performance} on suites of simulated data, where we know the truth and can therefore compare the performance of different approaches to the optimal can provide insight into when our theories are working as expected, including in settings that extend beyond the theoretical constraints.  Moreover, \textbf{real data performance}, both accuracy and time, provide insight into different methodologies when we do not know much about the true data generating mechanism.

Finally, in addition to the above mentioned quantitative criteria, there are also many qualitative criteria that merit consideration.  First, the \textbf{intuitiveness} of the method can provide a warm fuzzy feeling because the ways in which a particular method can go wrong are better understood.  Second, \textbf{hyper-parameter searches} can add significant computational overhead, as searching over many different hyper-parameters can exponentially increase the amount of time one spends estimating the optimal boundary. Third, whether the implementation is \textbf{free and open source software} (FOSS) impacts its availability, especially for non-profit organizations, developing world institutions, and citizen scientists.  Fourth, the \textbf{ease of use}, which is largely determined by platform considerations.  In particular, if running the method requires access to a supercomputer, or a Hadoop cluster, or non-free software (like MATLAB), or otherwise specialized software and hardware configurations,  it may require more activation energy and costs.  
...
Pseudocode of any method that embeds  data as part of classification proceeds as schematized in Figure \ref{f:mnist}:
...
For the first three, the top panels depict the means (top), the shared covariance matrix (middle), and these examples satisfy the assumptions underlying Theorem \ref{t:LDA}.  For the next four, the top panels depict a 2D scatter plot (left), mean and level set of one standard deviation of covariance matrix (right), and all four are examples for which our theory does not extend.  For all seven simulations, the bottom panel shows misclassification rate as a function of the number of embedded dimensions, for several different classifiers.  The simulations settings are as follows:
...
Bottom panels show the minimum error rate (over hyperparameters) achieved by each of the five algorithms versus time.
...
\textbf{(A)} Prostate: a standard sparse dataset.  1-dimensional \Lol~does very well, although \sct{Road} keeps $2^5$ ambient coordinates to slightly improves performance, at a significant cost of compute time (two orders of magnitude), with minimal additional interpretability.
...
For regression, \Lol~composed with linear regression outperforms \sct{Lasso}~(cyan), as well as partial least squares (PLS; black).
...
For example, one flavor of strategies, collectively called ``manifold learning'', typically assumes that the space that contains the signal is a linear or nonlinear combination of the original space (a rather general assumption) \cite{deSilva2003}.   More recently a large number of nonlinear manifold learning methods have been developed (e.g., \cite{Kohonen1982a, Bishop1998a, Tenenbaum2000a, Roweis2000a, Belkin2003a, Zhang2004f, Coifman2006a, Allard2012}), many of which can be thought of as kernel versions of PCA \cite{Ham2004a}, also called ``spectral methods''.


A complimentary flavor of strategies for dealing with wide data is regularization or shrinkage \cite{Friedman1989a,Bickel2004a,Bouveyron2007}, including smoothing \cite{Witten2009a} and more recently \emph{sparsity} \cite{Olshausen1997a}. Sparse methods assume that signal lives in a small subset of the ambient dimensions (e.g.,  \cite{Tibshirani1996, Donoho2008a, Fan2008, Li2012, Tibshirani2002a,Fan2008a,Witten2009a,Clemmensen2011a,Mai2013a,Fan2012a}).  This assumption, when accurate, can lead to substantial improvements in accuracy with relatively moderate increase in computational cost \cite{Efron2004}.  Moreover, it is comparably easy to incorporate supervised information. 
