...
\begin{abstract}
Supervised learning is a fundamental problem in statistics and pattern recognition.  In the modern era, it is routine to collect high-dimensional data without obtaining a large number of samples.  In such situations, some form of regularization or dimensionality reduction is required to achieve high-quality performance.
Because the objective function in such problems is highly nonlinear, current approaches typically solve a surrogate objective function, but lack any guarantees with regard to the task at hand, or require overly restrictive assumptions on the data, such as the sparsity. 
Using simple geometric arguments, we discover that combining the means and covariances in an intuitive fashion yields a low-dimensional projection that is nearly optimal in a wide variety of simulated and real data scenarios. By utilizing off-the-shelf numerical libraries, our method, called ``Linear Optimal Low-rank'' (LOL) is extremely computationally efficient as well. Theoretical analyses confirm our intuition.  
% We therefore position LOL as a potential new and principled ``default'' algorithm for any supervised learning problem with wide data.  
This work therefore opens the door for more general computationally and theoretically tractable supervised manifold learning algorithms.
% To enable reproducability and extendability, we provide open-source code in Matlab, R, and Python.  
\end{abstract}
