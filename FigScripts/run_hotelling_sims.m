function [T,S] = run_hotelling_sims(tasknames,task)
...
if ~isfield(task,'n'),      task.n=50; end
if ~isfield(task,'Nb'),     task.Nb=8; end
if ~isfield(task,'bvec'),   task.bvec=[0,logspace(-2,-1,task.Nb-1)]/2; end
if ~isfield(task,'transformers'), task.transformers= parse_algs({'NEA';'DEA';'NEN';'DEN'}); end
...
for t=1:length(tasknames)
    ...
    for k=1:T1.Nb
        ...
        for i=1:T1.Ntrials
            ...
            T1.b=T1.bvec(k);
            parms = set_parameters(T1);
            gmm = gmdistribution(parms.mu',parms.Sigma,parms.w);
            [X,Y] = random(gmm,T1.n);
...
if task.save
    save(['../../Data/Results/test_sims'],'T','S','task')
end
