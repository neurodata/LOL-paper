...
for j=1:length(T)
    ...
    if j==1     % if there is a 3rd class
        ...
        plot(Xplot3(1,idx),Xplot3(2,idx),'s','color',G.colors{3},'LineWidth',G.lw,'MarkerSize',2)
...
%%
pos=[left+(4-1)*(width+hspace)+0.04 bottom+(height+vspace)*4+0.06 width height]; %[left,bottom,width,height]
...
l=legend(g,'LOL','LDA o PCA','ROAD','QOQ','ROL','QDA o \delta+RP','QDA o RP');
...
str = {'total # of embedded dimensions'};
annotation('textbox', [0.35,0.01,0.6,0.04],'String', str,'EdgeColor','w'); %[x y w h]
