...
elseif strcmp(task.subname,'p=2D')
    X=randn(D,n);
    p=200; % nnz
elseif strcmp(task.subname,'toeplitz') % stability selection (c)
    rho=0.99;
    c=rho.^(0:D-1);
    A = toeplitz(c);
    ...
    p=100; % nnz
    eps2=4;
elseif strcmp(task.subname,'squared') % stability selection (a), 
    X=randn(D,n);
    p=100; % nnz
    func = @(x) x.^2;
elseif strcmp(task.subname,'sin') % stability selection (a), 
    X=randn(D,n);
    p=100; % nnz
    func = @(x) sin(x);
end
