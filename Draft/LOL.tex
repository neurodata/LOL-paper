...
Technically, to maintain orthonormality, we must orthonormalize, $\bA^{LOL}_d=$ \sct{Orth}$([\bdel, \bA^{PCA}_{d-1}])$.  Below, we show that this orthonormalization does not matter very much.
...
% First, note that for any distribution $F \in \mc{F}_{LDA}$, we can reparameterize it such that $\bSig$ is diagonal.  This follows because for any $\bSig$, we can represent it as $\bSig=\bU \bD \bU$, and there exists a $\bW$ such that 
% from the following:
% \begin{align}
% \bSig = \bU \bD \bU = 
% \end{align}
...
% 
% \begin{lem}
% $L_{A \circ \Fld}^F = L_{\circ \Fld}^{W \circ F}$, for any $F \in \mc{F}$.
% \end{lem}
...
% Two immediate corrolaries of the above that we state without proof are the composing \Fld~with either \Pca~or \Lol~ is rotationally invariant:
% \begin{cor}
