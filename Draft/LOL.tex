...
$A = \delta$ and $B = e_1 = (1,0,0,\dots,0)$ be the linear transformations for LOL and PCA of $X$ into $\mathbb{R}$. We then have  
\begin{gather*}
...
When $A = \bigl[ \delta \mid e_1 \mid e_2 \dots \mid e_{d-1} \bigr]$ and $B = \bigl[e_1 \mid e_2 \mid \dots \mid e_d \bigr]$ are the linear transformation for LOL and PCA of $X$ into $\mathbb{R}^{d}$, we have
\begin{gather*}
C(F_0^{(A)}, F_1^{(A)}) = \frac{(\sum_{i=d}^{p} \delta_i^2)^2 + \sum_{i=1}^{d-1} \lambda_i^{-1} \delta_i^2}{\sum_{i=d}^{p} \delta_i^2 \lambda_i};  \quad 
