...
Consider again the rotated trunk example as well as a ``Toeplitz'' example, as depicted in Figures \ref{f:properties}(A) and (B).  In both cases, the data are sampled from the \Lda~ model, and in both cases, the optimal dimensionality depends on the particular approach, but is never the true dimensionality.  Moreover, \Lol~dominates the other approaches, regardless of the number of dimensions used.
% 
Figure \ref{f:properties}(C) shows a sparse example with ``fat tails'' to mirror real data settings better.  The qualitative results are consistent with those of (A) and (B).  Indeed, we can generalize Theorem \ref{t:n} to include ``sub-Gaussian'' data, rather than just Gaussian:
...
\section{Rotational Invariance of LDA based Classifiers}
...
For simplicity (but without loss of generality), we will also assume that the two classes
...
\begin{question} \label{q:angle}
Is it true that:
\begin{lem} \label{l:angle}
$$\angle(\bA,\bA_*) \leq \angle(\bA',\bA_*) \implies \angle(\bP_A,\bP_*) \leq \angle(\bP_{A'},\bP_*).$$
\end{lem}
...
Note that Corollary \ref{l:angle} is a stronger statement than Lemma \ref{l:angle}, and in particular, Corollary \ref{l:angle} implies Lemma \ref{l:angle}.
\end{answer}
...
\begin{question}
When $\bSig=k \mb{I}$, under what conditions is \Lol~better than \Pca?
\end{question}

\begin{answer}
Almost always.  In particular, \Lol~is the optimal projection vector when $d=1$. $\bA_{\Lol}^1 = \bA_*$
\end{answer}
...
And by Lemma \ref{l:angle}, $\angle(\bP_A,\bP_*)=\angle(\bP_{c A},\bP_*)$, for any $c>0$, and in this scenario, $\bA_{\Lol}^1=c \bA_*$.
\end{proof}
