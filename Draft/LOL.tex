...
Supervised learning (SL)---the art and science of discovering statistical relationships between data of multiple different measurement types---is one of the most useful tools in the scientific toolbox.  SL has been enabled a wide variety of basic and applied findings, ranging from discovering biomarkers in omics data \cite{Vogelstein2014a}, to object recognition from images \cite{Krizhevsky2012}, and includes celebrated methods such as support vector machines, random forests, and deep networks \cite{Hastie2004}.  A special case of SL is classification; a classifier can predict the 'class' of a novel observation via training on a set of paired observations and class labels (for example, predicting male vs. female from MRI scans).   In such problems, the goal is to find the optimal discriminant boundary, which partitions the space of observations into the different classes.  In this big data age, the ambient (or observed) dimensionality of the observations is quickly ballooning, and with it, the ambient dimensionality of the discriminant boundary.  While historical data may have consisted of only a few dimensions (e.g., height and weight), modern scientific datasets often consist of hundreds, thousands, or even millions of dimensions (e.g. genetics, neuroscience, omics). Regardless of the dimensionality, when a scientist or analyst obtains a new dataset consisting of some observations and labels, she must decide which of the myriad available tools to use. %{Reference} algorithms for datasets with low dimensionality include linear and quadratic discriminant analysis, support vector machines, and random forests \cite{Hastie2004}.

Two crucial factors to consider when choosing a tool include statistical and computational.  Statistical considerations can be characterized by the   ``bias-variance tradeoff''.  Bias, the mismatch between assumptions characterizing the data and the data itself, provides a \emph{lower bound} on the  error.  Variance, the size of the set of possible answers,  provides an \emph{upper bound} on the error. The statistical  goal for a given new challenge problem is to choose a methodology that jointly minimizes both bias and variance.  Computationally, one desires a method that is numerically stable (which is related to low variance) and computationally efficient.


Classical statistical methods often assume that the dimension that the number of samples $n$ is much larger than the dimensionality of the problem $p$.  For ``wide data'' problems where $p \gg n$ this assumption both incurs a high bias and a high-variance  because $p$ is so large.   The implication of the high variance cost are substantial. An infinite number of estimators work equally well on any size training data, so the algorithm will choose one essentially at random, with little or no predictive power or accuracy outside of the training data. That is assuming that the algorithm can run, because it will use lots of memory, possibly be numerically unstable, and often might not even provide an answer at all.

To reduce variance without incurring a significant bias one can make certain ``structural'' assumptions about the data.  All such structural assumptions essentially impose that the signal of interest lies near a lower dimensional space or manifold, the differences lies in the nature of those assumed spaces/manifolds.  For example, one flavor of strategies, collectively called ``manifold learning'', typically assumes that the space that contains the signal is a linear or nonlinear combination of the original space (a rather general assumption).  The simplest forms of manifold learning are linear, including both  multidimensional scaling \cite{Householder1938,Borg2005} and principal components analysis (PCA) \cite{Pearson1901a,Jolliffe2002a}.  More recently a large number of nonlinear manifold learning methods have been developed (e.g., \cite{Kohonen1982a, Bishop1998a, Tenenbaum2000a, Roweis2000a, Belkin2003a, Zhang2004f, Coifman2006a, Allard2012}), many of which can be thought of as kernel versions of PCA \cite{Ham2004a}, also called ``spectral methods''.

The beauty of these approaches is that they have strong theoretical guarantees \cite{deSilva2003},  and linear variants can be incredibly numerically efficient \cite{Zheng2016}; their downfall however is that they are fully \emph{unsupervised}, meaning that in regression and classification problems they do not use all of the data to their advantage.  In particular, the above algorithms seek to discover the directions of maximal variance (in a specified metric space).  Unfortunately, in many supervised learning problems---and in classification in particular---the directions of maximal variance may be orthogonal to the directions that contain the signal of interest. Indeed, in general, there is no a priori reason to believe that these two sets of directions will be well aligned with one another.  ``Supervised dictionary learning'' tries to incorporate supervised information \cite{Mairal2009}.  However, to gain supervised information, these methods lose both theoretical guarantees and efficient numerical algorithms.


A complimentary flavor of strategies for dealing with wide data is regularization or shrinkage \cite{Friedman1989a,Bickel2004a,Bouveyron2007}, including smoothing \cite{Witten2009a} and more recently \emph{sparsity} \cite{Olshausen1997a}. Sparse methods assume that signal lives in a small subset of the ambient dimensions (e.g.,  \cite{Tibshirani1996, Donoho2008a, Fan2008, Li2012, Tibshirani2002a,Fan2008a,Witten2009a,Clemmensen2011a,Mai2013a,Fan2012a}).  This assumption, when accurate, can lead to substantial improvements in accuracy with relatively moderate increase in computational cost \cite{Efron2004}.  Moreover, it is comparably easy to incorporate supervised information.  However, the sparse assumptions are difficult to assess on a given dataset \cite{Tillmann2014}, and rarely match the structure of the data, and even when they do these methods still struggle to find the signal \cite{Su2015}.



A gap therefore persists in the literature: we desire a method that achieves both good statistical properties---theoretical guarantees for the relevant task under relatively general assumptions resulting in both small bias and variance---and computational properties.  Our approach,  ``Linear Optimal Low-rank'' (\Lol) embedding satisfies these desiderata.   The motivation for \Lol~comes from a simple geometric intuition:  in simple classification problem, the class conditional means and covariances are sufficient statistics.  But the entire covariance matrices are too high dimensional to directly estimate.  Therefore, we regularize it via only keeping the first several principal components, a common strategy \cite{Belhumeur1997a}. To remove much of the bias, we therefore also include the means, yielding a kind of supervised \Pca, that improves upon \Pca~for supervised learning.  In particular, under relatively general assumptions, the learned LOL space is closer to the optimal space than \Pca~space for any fixed dimensionality.  Moreover, \Lol~inherits all the computational advantages of \Pca, unlike dictionary learning and sparse methods.  \Lol~exhibits these advantages even in more general contexts than we have theoretical guarantees for.  By virtue of developing \Lol~via geometric intuition, it is straightforward to extend it to more general settings. 
Finally, \Lol~can also be used to improve high-dimensional regression and testing.
