...
Classical methods often rely on very restrictive assumptions that compromise their performance. In particular, the theoretical guarantees upon which many classical methods rest require that the number of samples is much larger than the dimensionality of the problem ($n \gg p)$.  
...
When the number of dimensions is orders of magnitude larger than the sample size, as is now typical, this $n \gg p$ assumption is woefully inadequate. XXX remove woefully inadequate, add results in ..... a compressed version of the next 2 sentences XXX
...
%%% XXX relaxed is a bit jargony, just say we built methods to do this stuff. 

%%% 1st sentence should contain the whole idea of the paragraph

%%% define bias at first use (keep the beauty)

%%% swap 2nd & 3rd paragraphs? or relate bias/variance to 2nd paragraph, or vice versa.

To combat these issues, the fields of statistics and machine learning have developed a  collection of new methods that relax these assumptions, and exhibit significantly improved performance characteristics in these settings.  Each such approach makes some ``structural'' assumptions about the data (sometimes in the form of priors), therefore potentially adding some bias, while reducing variance.  The best approach, for a given problem, is the approach that wins this bias/variance trade-off; that is, the approach whose assumptions best correspond to the properties of the data (minimizing bias), and yields estimates have that bound error under those assumptions (minimizing variance). These approaches complement ``feature engineering'' approaches, where the practitioner designs new features based on prior domain specific knowledge.
