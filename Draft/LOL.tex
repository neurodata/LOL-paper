...
\subsection*{Linear Optimal Low-rank}
...
\caption{Schematic of Linear Optimal Low-rank Embedding using 300 samples of the numbers 3, 7, and 8 from the MNIST dataset; each sample is a 28 $\times$ 28 = 784 dimensional image.
...
\textbf{(A)} A subset of training samples  (boundary colors are  for visualization purposes).
\textbf{(B)} The first four projection matrices learned by \Lol~on 100 training samples from each class.
...
\textbf{(C)} Projecting 500 test samples into the top two learned dimensions.
 % using each approach.
 Digits color coded as in (A).
\textbf{(D)}  The estimated posterior distributions for 3 and 8 of test samples after
projecting data into two dimensions and using LDA to classify.
...
(B) learn a ``projection'' that converts the high-dimensional data to a low dimension representation,
