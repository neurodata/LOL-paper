...
\begin{figure}
\centering % l b r t
% \includegraphics[width=1\linewidth,trim=0cm 0cm 1cm 4.4cm,clip=true]{../Figs/mnist.pdf}
...
% \begin{SCfigure}
\centering
\includegraphics[width=0.6\linewidth,trim=0in 0in 1.5in 0in,clip=true]{../Figs/cigars_est.pdf}%l b r t
\caption{
...
\textbf{Row 3}: \sct{Road}, a sparse method designed specifically for this model \cite{Fan2012}.
...
\begin{figure}
\centering
\includegraphics[width=1\linewidth,trim=0.5in 4.5in 0.5in 0.5in,clip=true]{../Figs/table.pdf} %l b r t
...
% Note that  $g^*(\bx):=\argmax_y f_{\bx|y}f_y$ can be written as 
% $\II\{f_{\bx|0}f_{y=0} < f_{\bx|1}f_{y=1}\}$.  Taking the log of both sides, we have:
% $\II\{\log f_{\bx|0} \log f_{y=0} < \log f_{\bx|1} \log f_{y=1}\}$.  For simplicity, we let $f_{y=0}=f_{y=1}$.  Rearranging terms, we are left with
% $\II\{\log f_{\bx|0} - \log f_{\bx|1} < 0 \}$.
% Plugging in the \Lda model assumption, the argument becomes
% \begin{align*}
% -\frac{1}{2} (\bx-\bmu_0)\T \bSig^{-1}(\bx-\bmu_0) 
% \frac{1}{2} (\bx-\bmu_1)\T \bSig^{-1}(\bx-\bmu_1). 
% \end{align*}
% Expanding the terms and re-arranging a bit, and assuming that the data are centered---$(\bmu_0+\bmu1)/2=0$---we obtain:
\begin{align*}
...
% \begin{align} \label{eq:\Lol>\Pca}
$L_{\Lol}^d \leq L_{\Pca}^d  \forall \, \bth \in \bTh_{2-\Lda}$,  and  any $d < p$.
% \end{align}
 \end{thm}
...
\para{I DON'T THINK THIS SUBSECTION IS CORRECT}
...
To obtain the first principle component, consider the general definition of variance.
The distribution $\bX$ is actually mixture of Gaussians:
$\bX \sim \sum_j \pi_j \mc{N}(\bmu_j,\bSig)$.  
Because  $\mu_0=-\mu_1$, it is the case that $\mu_0^2=\mu_1^2$.
...
$X_i \sim \sum_j \pi_j \mc{N}(\mu_{ij},\sigma_i^2)$, where 
var($X_i$)$=\sum_j \pi_j \mu_{i,j}^2 / \sigma_i^{2}=\sum_j \pi_j \mu_i^2 / \sigma_i^2=\mu_i^2 / \sigma_i^2$,
where the  equalities follow from the equal class priors and centered mean assumptions. 
% Recall that $\mt{\bdel}=(2 \mu_1/\sigma_1^2,\ldots, 2 \mu_p/\sigma_p^2)$. 
Let $i_*=\argmax_i \mu_i^2 / \sigma_i^2$. % and $\mt{\del}_{(i)}=\max_i \mu_i^2 / \sigma_i^2$.
...
% \begin{lem}
% $\bA_{\Pca_1}=\bA_{\Lol_1}=\bA_*=\mt{\bd}$ when $\bSig=\bS$, $\pi_0=\pi_1$, and $\bmu_0-\bmu_1=\mb{0}$.
% \end{lem}
% \begin{proof}
% Recall that $\mt{\bd}=\bd \odot \bdel$, where $\bd$ is the diagonal of $\bS$, and $\odot$ is the Hadamard (elementwise) product.  By definition, the first dimension of \Lol~is $\mt{\bd}$, which proves the second equality.
...
\begin{lem}
$L_{\Lol}^d \leq L_{\Pca}^d$ when $\bSig=\bI$ and $\pi_0=\pi_1$ and $(\mu_0+\mu_1)/2=\mb{0}$.
\end{lem}
...
% \begin{cor}
% $L_{\Lol}^1 \leq L_{\Pca}^1$ when $\bSig=\bI$ and $\pi_0=\pi_1$.
% \end{cor}
