...
\title{\vspace{-50pt}\db{Big Supervised Manifold Learning by Linear Optimal Low-Rank (LOL)}}
\author{Joshua T.~Vogelstein, Brett Mensh, Minh Tang, Mauro Maggioni}
\date{}
...
In this big data age,  the ambient (or observed) dimensionality of the observations is quickly ballooning, and with it, the ambient dimensionality of the discriminant boundary.  While historical data may have consisted of only a few dimensions (e.g., height and weight), modern scientific datasets often consist of hundreds, thousands, or even millions of dimensions (e.g. genetics, neuroscience, omics). Regardless of the dimensionality, when  scientists or analysts obtain new datasets, they must decide which of the myriad available boundary estimation methods (estimators) to use.  When choosing an approach, there are many factors to consider across different levels of analysis, including both statistical and computational considerations. 

The goal of such problems is to estimate a discriminant boundary that optimally partitions the space. To do so, one must make a series of decisions to minimize the expected error in a reasonable amount of time.  These decisions have a variety of ramifications; the art of supervised learning is making choices that balance those ramifications appropriately for the problem at hand.  The choices at each of the four levels of analysis: (i) problem specification, (ii) algorithm, (iii) implementation, and (iv) platform,  have different ramifications, both quantitative and qualitative implications.  The quantitative impact can be further subdivided into statistical and computational implications, with statistical implications largely determined by the top two levels, and computational implications governed by the bottom two.   
% Those choices will be based jointly on a number of \emph{modeling assumptions} regarding the data and \emph{computational constraints} on the procedure.  In addition to the above mentioned quantitative factors, there are a number of other qualitative considerations, such as interpretability and ease of use that will eventually determine the choice of approaches.  
Below, we list in bold the \emph{choices} one must make, and in italics the \textbf{ramifications} of each.  
% These choices span the four different levels of analysis: (i) problem specification level, (ii) algorithmic level, (iii) implementation level, and (iv) platform level.  
Associated with each level are trade-offs, as we will elaborate upon below.  
% The first two levels are concerned with statistical considerations, whereas the latter two levels deal with computational considerations.

% \textbf{Problem Specification:}
At the problem specification level, one must formally state the problem.  This statement is determined by two different kinds of model assumptions \cite{Breiman2001b}: \emph{data generating model} and \emph{algorithm model} (often called feasible region or search space). These modeling assumptions determine geometric constraints on the shape of the optimal boundary, and the estimated boundary, respectively.  
For example, Gaussian is a data generating model, which means that the actual data generating mechanism would be a Gaussian with a specified the mean and covariance. This geometric assumption of the data generating mechanism implies constraints on the optimal discriminant boundary, which is characterized by (a potentially multidimensional) parameter.  Similarly, a linear boundary is a potential feasible region, and the particular optimal boundary is then specified by (a potentially multidimensional) parameter: the slope and intercept. The mismatch between these two models  determines the \textbf{model bias}, which formally is the minimum distance between the actual data generating mechanism boundary parameter and the optimal estimator of said boundary within the feasible region.  While ideally these is no mismatch, there are many reasons for mismatch to occur in real data problems.  First, while it is relatively easy to assume different data generating models, the implied space of optimal boundaries are often difficult or impossible to determine analytically.  Thus, the model bias is in general unknown without making particularly well understood assumptions.  Second, the larger the assumed data generating model, the larger the search space must be to be sure to include the optimal boundary.  But searching large spaces can be computationally prohibitive.  


% First, one can choose which assumptions to make about the \emph{data generating mechanism}.  These properties imply geometric constraints on the set of optimal discriminant boundaries.  For example, if one assumes that the data are sampled from a Gaussian, and the two classes differ only in their means, then one can prove that the optimal discriminant boundary is linear, but we do not know the (potentially multidimensional) slope and intercept.  Second, given such assumptions, one can then choose the \emph{boundary search space} over which one will search for the optimal boundary.  For example, if we assume the optimal boundary is linear, we can search only for linear boundaries, and exclude multi-linear and quadratic boundaries.  The minimum distance between the boundaries we search over and the optimal boundary is called the \textbf{model bias}, which we want to minimize by choosing a search space as close as possible to the optimal boundary space.  

At the algorithm level, one must choose  an \emph{algorithm} to find an optimal boundary, such as Fisher’s Linear Discriminant, Linear Support Vector Machines, or Logistic Regression.  Each such algorithm has different convergence properties as sample size increases.  Importantly, for different data generating mechanisms, different algorithms may converge on different solutions.  The additional distance between the expected estimated parameter and the optimal parameter is called the \textbf{estimator bias}, which we also want to minimize by choosing an algorithm that will select an estimator as close as possible to the truth.  Different algorithms will also differ in their \textbf{estimator variance}, which is a function both of the size of the search space, and the idiosyncrasies of the algorithm.  Of note, in certain circumstances, the two bias terms and the variance together determine expected square error, so one also desires to minimize the variance of the estimated boundary. 
In addition to the estimator bias and variance, \textbf{robustness} to model misspecification and in particular outliers is often a very important property for real data problems. 


% In addition to the above statistical considerations, there are a number of computational considerations to consider which also depend on a series of choices. In this big data age, the practical importance of these considerations is quickly gaining on the importance of the various statistical considerations.  
At the implementation level, one must choose an \emph{implementation}, which can have dramatic computational implications.  First, implementations should be \textbf{numerically stable}, over the range  of the number of samples and dimensions under consideration.  Second, one desires an implementation that minimizes computational complexity, both in terms of worst case \textbf{space} and \textbf{time}. For most algorithms, in addition to the parameters that are estimated, there are additional parameters that are typically manually tuned.  These ``hyper-parameters'' include things like the sparsity level, the number of dimensions, the kernel function, or the number of neurons.  The precise choice of these hyper-parameters can have a dramatic impact on the algorithm's performance. The ease with which one can perform \textbf{hyper-parameter searches} can have a dramatic impact on both the accuracy and efficiency of a particular implementation. 
% Estimator Bias and variance comprise the statistical (quantitative) considerations for choosing an algorithm, the most important qualitative consideration is \textbf{hyper parameter searches}.  In particular, all algorithms have a number of parameters that can easily be estimated, and others that are typically manually tuned.  The number of different parameter to manually tune, and the ease with which each can be ``swept over'' to find an optimal one varies dramatically across different algorithms, and is can therefore be an important additional consideration.



At the platform level, one must choose the \emph{platform} upon which the implementation will operate, including both software dependencies and hardware choice. In particular the \textbf{scalability} of the platform, that is, how large of a problem can be solved on a particular platform has important ramifications. 


All of the above quantitative considerations can be evaluated theoretically, but typically only under restrictive assumptions.  Moreover, theoretical results are limited to certain relatively simple sets of assumptions, assumptions for which many real data sets and algorithms do not satisfy. Even when the assumptions are met, theory often merely provides bounds, without concrete numbers in any particular situations. Numerical experiments can therefore buttress the theoretical considerations.  In particular, \textbf{simulated performance} on suites of simulated data, where we know the truth and can therefore compare the performance of different approaches to the optimal can provide insight into when our theories are working as expected, including in settings that extend beyond the theoretical constraints.  Moreover, \textbf{real data performance}, both in terms of accuracy and time, provide insight into different methodologies when we do not know much about the true data generating mechanism.





Finally, in addition to the above mentioned quantitative criteria, there are also many qualitative criteria that merit consideration at each level of analysis.  For the problem specification level,  the model choices also determine the  \emph{appropriateness}.  Indeed, real decision criteria are often quite difficult to specify precisely, and so our problem specification is almost always related to but not exactly the problem we hope to achieve.  For example, while we might want to parse a scene, we often tackle a simpler problem of checking whether an object of a particular kind is present because it is easier.  For the algorithm level, the \textbf{interpretability} of the resulting estimator is often an important consideration.  For example, when developing biomarkers, it is important that the doctors and insurance companies can understand the decision making process of the algorithm, otherwise they might not believe the results. For the implementation level,  how \textbf{accessible} is the implementation is crucial.  In particular as more people outside the ivory academic tower---such as citizen scientists and even academics in the developing world---are contributing to data science and scientific discovery in general, access to the implementation is increasingly important.  Finally, at the platform level, the \textbf{ease of use} of the platform is increasingly important.  For example, requiring tedious installations of many different software libraries, or specialized hardware, can make the platform difficult to employ.  


The ideal method therefore, in addition to receiving high marks on all quantitative criteria, is appropriate for the problem, results in an interpretable estimator, with a free and open source implementation that is easy to install, or is available on  pre-configured virtual machines  that can run on standard local or cloud machines computing environments.



XXX no mention really of big sample sizes anywhere

XXX no mention of supervised manifold learning.



 % First, the \textbf{intuitiveness} of the method can provide a warm fuzzy feeling because the ways in which a particular method can go wrong are better understood.  Second, \textbf{hyper-parameter searches} can add significant computational overhead, as searching over many different hyper-parameters can exponentially increase the amount of time one spends estimating the optimal boundary. Third, whether the implementation is \textbf{free and open source software} (FOSS) impacts its availability, especially for non-profit organizations, developing world institutions, and citizen scientists.  Fourth, the \textbf{ease of use}, which is largely determined by platform considerations.  In particular, if running the method requires access to a supercomputer, or a Hadoop cluster, or non-free software (like MATLAB), or otherwise specialized software and hardware configurations,  it may require more activation energy and costs.  

It is quite a challenge to optimize along all of the above dimensions for any particular kind of exploitation task.  Here, we first focus on the simplest big data supervised learning task: two-class classification, where each class is sampled from a Gaussian distribution and they only differ in their means, and the dimensionality $p$ is greater than the sample size $n$, that is, $p > n$. Fisher’s Linear Discriminant (FLD), while asymptotically optimal (zero bias and variance as n goes to infinity), is numerically unstable when $n < p$, and therefore will not even run.  A very natural “fix” to this is to first ``pre-process'' the data via a truncated Principal Components Analysis (tPCA), and then apply FLD (an approach called Fisherfaces \cite{}.  While first running tPCA greatly reduces the variance relative to directly running FLD, and therefore makes the algorithm numerically stable, it adds estimator bias because the dimension reduction performed by PCA does not consider the class labels, only the predictor variables, and the discriminant boundary is determined by the relationship between the class labels and predictor variables. 
