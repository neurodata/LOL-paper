function [Wmin,d,f] = lrtLAD(Yaux,X,morph,parameters)
% 
% [Wmin,d,f] = lrtLAD(Y,X,morph,parameters);
% 
% This function estimates the dimension of the central subspace that best
% describes the data under the LAD model using a likelihood-ratio test.
% USAGE:
%  - outputs:
...
%--- get handle to objective function, derivative, dof, dof for CHI2.......
Fhandle = F(@F4lad,data_parameters);
dFhandle = dF(@dF4lad,data_parameters);
dof = @(do) (p+(h-1)*do+do*(p-do)+(h-1)*do*(do+1)/2 + p*(p+1)/2);
f0 = n*(p*(1+log(2*pi)) + logdet(sigmag))/2;

%---- chooses dimension
lrt = lrtloop_handle_vf(Y,X,data_parameters,parameters);
d = lrt(Fhandle,dFhandle,f0,dof);

%---- perform LDA with the selected dimension
if d>0,
