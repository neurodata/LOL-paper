function [Wn,dim,fn] = lrtIPFC(Yaux,X,morph,parameters)
%
% [Wmin,d,f] = lrtIPFC(Y,X,morph,parameters);
% 
% This function estimates the dimension of the central subspace that best
% describes the data under the IPFC model using a likelihood-ratio test.
% USAGE:
%  - outputs:
...
lo = n*p/2*(1 + log(trace(SIGMAres/p))); 
%----
[Waux,valsA] = firsteigs(SIGMA,p);
[W,vals] = firsteigs(inv(SIGMAres)*SIGMA,p);
for u=0:min(r,p),
    if u==0,
        fn = n*p/2*(1 + log(sum(valsA)/p));
    else
        fn = Fhandle(vals(1:(u+1)));
    end
    statistic = 2*(fn-lo);
